...
function _strong(d3,output9,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 700;
  const height = 500;
  ...
  // 計算圖形並啟動力模擬。
  const root = d3.hierarchy(output9);
  ...
  const linkForce = d3.forceLink(links)
    .id(d => d.id)
    .distance(1500) // 增加連結的距離
    .strength(2); // 設定連結的強度

  const myColors = ["#D0104C", "#E3916E", "#81C7D4"]; // 自定義的顏色
  ...
  const node = svg.append("g")
      ...
      .on("mouseenter", (event, d) => {
        if (d.data.leval === 2 || d.data.leval === 3) {
          ...
          const transition = d3.transition().duration(300).ease(d3.easeCubic); // 過渡
      ...
      .on("mouseleave", (event, d) => {
        const transition = d3.transition().duration(200).ease(d3.easeCubic); // 過渡
...
function _output9(FileAttachment){return(
FileAttachment("output9@1.json").json()
)}
...
function _output91(FileAttachment){return(
FileAttachment("output9@1.json").json()
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["output9@1.json", {url: new URL("../output9.json", import.meta.url), mimeType: "application/json", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("strong")).define("strong", ["d3","output9","drag","url_list","invalidation"], _strong);
  main.variable(observer("output9")).define("output9", ["FileAttachment"], _output9);
  main.variable(observer("url_list")).define("url_list", _url_list);
  main.variable(observer("drag")).define("drag", ["d3"], _drag);
  main.variable(observer("output91")).define("output91", ["FileAttachment"], _output91);
  return main;
}
